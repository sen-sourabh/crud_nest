...
export class Inventory extends Document {
    @ApiProperty({
        description: 'Unique item id of item with the prefix of `IN_` followed by ISO Date string that generates dynamically',
    ...
    @ApiProperty({
        description: 'Valid positive quantity of the inventory that should be greater than 0 (> 0)',
        example: '76',
    })
    @IsNumber({ allowNaN: false, allowInfinity: true, maxDecimalPlaces: 0 }, {
        message: 'Quanity must be a valid positive number that should be greater than 0 (> 0)',
    })
    @IsOptional()
    @Prop({
        required: false,
    ...
    @ApiProperty({
        description: 'Is refrigeration is needed for the inventory, false by default',
        example: 'false',
    })
    ...
    @ApiProperty({
        description: 'Location of the inventory in the real world, like drawer, library',
        example: 'drawer',
    })
    ...
    @IsNumber({ allowNaN: false }, { message: 'Minimum stock level must be a string' })
    @IsOptional()
    @Prop({
        required: false,
        type: 'number',
    ...
    @IsNumber({ allowNaN: false }, { message: 'Maximum stock level must be a string' })
    @IsOptional()
    @Prop({
        required: false,
        type: 'number',
